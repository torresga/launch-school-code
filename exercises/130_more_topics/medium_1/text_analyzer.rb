...
# pass text to the block provided
class TextAnalyzer
...
analyzer.process("analyzer_text.txt") do |lines|
  puts "#{lines.split("\n\n").size} paragraphs"
end

analyzer.process("analyzer_text.txt") do |lines|
   # your implementation - count lines
   puts "#{lines.split("\n").size} lines"
end

analyzer.process("analyzer_text.txt") do |lines|
  # your implementation - count words
  puts "#{lines.split.size} words"
end
