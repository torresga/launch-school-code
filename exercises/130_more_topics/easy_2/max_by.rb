...
# Input: An array
# Output: a number that represents the largest element in an array
# We are comparing our values by the return value of the block

# return nil if array is empty

# We want to return element in array, but we want to compare by block

# loop through array
  # collect return value from block inside a variable
  # p variable

  # need two variables, one for largest_element
  # one for largest_block_value

def max_by(arr)
  return nil if arr.empty?

  largest_element = arr.first
  largest_block_value = yield(largest_element)

  arr[1..-1].each do |item|
    block_value = yield(item)

    if block_value > largest_block_value
      largest_element = item
      largest_block_value = block_value
    end
  end
