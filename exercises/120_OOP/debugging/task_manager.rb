...
class TaskManager
  ...
  # Inside this method, we are variable shadowing - creating a new local variable called tasks. and instead of referring to the tasks get method after the assignment, we are referring to the local variable tasks. so tasks equals nil. and tasks.select raises an error because NilClass must have a private method called select.
...
valentinas_tasks.display_all_tasks
p valentinas_tasks.display_high_priority_tasks
