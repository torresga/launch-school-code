...
class Card
  ...
  # override the <=> method
  def <=>(other)
    ranks = [rank, other.rank]
    ranks.map!(&:to_s) if ranks.any? { |rank| rank.is_a?(Integer) }

    if ranks.include?("Jack") && ranks.include?("Ace") ||
       ranks.include?("Queen") && ranks.include?("King") ||
       ranks.first == "Ace" && %w(Jack Queen King).include?(ranks.last) ||
       ranks.last == "Ace" && %w(Jack Queen King).include?(ranks.first) ||
       ranks.first == "10" && %w(2 3 4 5 6 7 8 9).include?(ranks.last) ||
       ranks.last == "10" && %w(2 3 4 5 6 7 8 9).include?(ranks.first)
  ...
  # Have to override this so that we can test the values of each string
  def ==(other)
    to_s == other.to_s
  end
end
