...
# On line 30, one of these numbers in
# Math.log(number_of_documents / number_of_documents_with_term) should be a floating point number. Neither of these are, so if there's a fractional part when one is divided by the other, it is lost. That means that in many of these cases, the return value of idf is 0. Any number multiplied by 0 is 0. That gives a different result than if you have a number multiplied times a fractional number.
...
# Have to watch what you're dividing inside of the idf method - you can possibly get Infinity or NaN
# If the search term doesn't appear in the documents and you divide by 0.0 gives you infinity
# if don't have the correct length of documents or if the document doesn't exist and you divide 0 by 0.0 gives you NaN
